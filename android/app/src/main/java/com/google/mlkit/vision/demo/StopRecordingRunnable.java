...
public class StopRecordingRunnable implements Runnable {
    ...
    public void CreateVideo(){
        ...
        cameraSource.numFrames = 0;
        //Take photos in file and convert to mp4 using FFmpeg binary executable
        int rc = FFmpeg.execute("-y -framerate " + cameraSource.frameRate +
                " -start_number 1 -i sdcard/DCIM/Camera/saved_images/img"
                + cameraSource.num +"_" +"%d.jpg -i "
                + cameraSource.fileName + " sdcard/DCIM/Camera/final"
                + cameraSource.num + ".mp4");
        ...
        if (rc == RETURN_CODE_SUCCESS) {
            ...
            File audio = new File(cameraSource.fileName);
            audio.delete();
